...
func requireClientOwnershipHTTP(w http.ResponseWriter, claims *util.Claims, clientID uint) bool {
	if claims == nil {
		return true
	}
...
func requireClientBasicHTTP(w http.ResponseWriter, claims *util.Claims, clientID uint) bool {
	return requireClientPermissionHTTP(w, claims, models.PermClientBasic) &&
		requireClientOwnershipHTTP(w, claims, clientID)
}
